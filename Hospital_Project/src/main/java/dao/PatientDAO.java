...
public class PatientDAO {
	...
	public PatientVO selectPatient(Map<String, Object> map) {
		PatientVO vo = sqlSession.selectOne("patient.select_patient", map);
		return vo;
	}
	
	//아이디로 회원조회(인덱스만)
	public int selectPatientIdx(String pat_id) {
		int pat_idx = sqlSession.selectOne("patient.select_patient_idx", pat_id);
		return pat_idx;
	}
	
	//인덱스로 회원조회
	public PatientVO selectPatientByIdx(int pat_idx) {
		PatientVO vo = sqlSession.selectOne("patient.select_patient_by_idx", pat_idx);
		return vo;
	}
